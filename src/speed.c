...
int run_speed(caesar_t *aead, unsigned long long mlen, unsigned long long adlen,
              clock_t limit) {
  int ret, i, sta, stb;
  ...
  do {
    for (i = 0; i < sta; i++) {
      clen = 0;
      ret = aead->encrypt(ct, &clen, pt, mlen, ad, adlen, nsec, npub, key);
      if (ret != 0 || clen <= 0) {
  ...
  do {
    for (i = 0; i < sta; i++) {
      ret = aead->decrypt(pt, &t, osec, ct, clen, ad, adlen, npub, key);
      if (ret != 0) {
...
int test_speed(caesar_t *aead, int limit) {
  ...
  if (brutus_verbose) {
    printf("[%s] Speed Test (limit=%d sec)  "
           ...
           aead->name, limit, aead->keybytes, aead->nsecbytes, aead->npubbytes,
           aead->abytes);
  }
