...
// Modifications Copyright (c) 2026 pasze888
...
#define HARDWARE_ACCELERATION_INTEL_AESNI
    ...
    !defined(USE_CXX_AES) && !defined(USE_INTEL_AESNI)
#if defined(__GNUC__)
#include <stdint.h>
#endif
...
namespace Cipher {
template <size_t key_bits = 128> class Aes {
  ...
  void AES_128_Key_Expansion(const unsigned char *user_key,
                             unsigned char *key) {
    __m128i tmp1, tmp2;
  ...
  void AES_192_Key_Expansion(const unsigned char *user_key,
                             unsigned char *key) {
    ...
    key_sched[1] = _mm_castpd_si128(_mm_shuffle_pd(
        _mm_castsi128_pd(key_sched[1]), _mm_castsi128_pd(tmp1), 0));
    key_sched[2] = _mm_castpd_si128(
        _mm_shuffle_pd(_mm_castsi128_pd(tmp1), _mm_castsi128_pd(tmp3), 1));
    ...
    KEY_192_ASSIST(&tmp1, &tmp2, &tmp3);
    key_sched[4] = _mm_castpd_si128(_mm_shuffle_pd(
        _mm_castsi128_pd(key_sched[4]), _mm_castsi128_pd(tmp1), 0));
    key_sched[5] = _mm_castpd_si128(
        _mm_shuffle_pd(_mm_castsi128_pd(tmp1), _mm_castsi128_pd(tmp3), 1));
    ...
    KEY_192_ASSIST(&tmp1, &tmp2, &tmp3);
    key_sched[7] = _mm_castpd_si128(_mm_shuffle_pd(
        _mm_castsi128_pd(key_sched[7]), _mm_castsi128_pd(tmp1), 0));
    key_sched[8] = _mm_castpd_si128(
        _mm_shuffle_pd(_mm_castsi128_pd(tmp1), _mm_castsi128_pd(tmp3), 1));
    ...
    KEY_192_ASSIST(&tmp1, &tmp2, &tmp3);
    key_sched[10] = _mm_castpd_si128(_mm_shuffle_pd(
        _mm_castsi128_pd(key_sched[10]), _mm_castsi128_pd(tmp1), 0));
    key_sched[11] = _mm_castpd_si128(
        _mm_shuffle_pd(_mm_castsi128_pd(tmp1), _mm_castsi128_pd(tmp3), 1));
  ...
  void AES_256_Key_Expansion(const unsigned char *user_key,
                             unsigned char *key) {
    __m128i tmp1, tmp2, tmp3;
    __m128i *key_sched = (__m128i *)key;
    tmp1 = _mm_loadu_si128((__m128i *)user_key);
...
// space for arm neon variables in case needed in the future.
  ...
  void mix_columns(unsigned char state[AES_BLOCK]) noexcept {
    ...
    // TODO: analyse in the future if this loop can speed up by branchless
    // programming.
    for (size_t i = 0; i < 4; ++i) {
      for (size_t k = 0; k < 4; ++k) {
        gf_product[i * 4] ^=
            (cmds[i][k] == 1) ? state[k * 4] : gf[cmds[i][k]][state[k * 4]];
        gf_product[i * 4 + 1] ^= (cmds[i][k] == 1)
                                     ? state[k * 4 + 1]
                                     : gf[cmds[i][k]][state[k * 4 + 1]];
        gf_product[i * 4 + 2] ^= (cmds[i][k] == 1)
                                     ? state[k * 4 + 2]
                                     : gf[cmds[i][k]][state[k * 4 + 2]];
        gf_product[i * 4 + 3] ^= (cmds[i][k] == 1)
                                     ? state[k * 4 + 3]
                                     : gf[cmds[i][k]][state[k * 4 + 3]];
      }
    }
  ...
  void add_round_key(unsigned char state[AES_BLOCK],
  ...
  void xor_dword(unsigned char *dest, unsigned char *a,
...
public:
   ...
   * This key should either be **16, 24, 32** bytes, or `128`, `192`, `256`
   * bits.
   */
  Aes(unsigned char key[key_bytes]) : round_keys() {
    constexpr bool invalid_aes_key_bit_size =
        key_bits == 128 || key_bits == 192 || key_bits == 256;
    static_assert(invalid_aes_key_bit_size,
                  "The valid values are only: 128, 192 & 256");
  ...
  ~Aes() { std::memset(round_keys, 0x00, round_keys_size); }
  ...
  void encrypt_block(unsigned char *output, unsigned char *block) {
#ifdef HARDWARE_ACCELERATION_INTEL_AESNI
...
#else
    std::memcpy(output, block, 16);
    state_transpose(output);
    ...
    for (size_t round = 1; round <= Nr - 1; ++round) {
      ...
      shift_rows(output);
      mix_columns(output);
      add_round_key(output, &round_keys[round * Nb * 4]);
    }

    sub_bytes(output);
    shift_rows(output);
    add_round_key(output, &round_keys[Nb * Nr * 4]);
  ...
  ///
  /// @param block 16 byte block of memory.
  void decrypt_block(unsigned char *output, unsigned char *block) {
#ifdef HARDWARE_ACCELERATION_INTEL_AESNI
    // load the current block & current round key into the registers
    __m128i *xmm_round_keys = (__m128i *)round_keys;
    __m128i state = _mm_loadu_si128((__m128i *)&block[0]);
    ...
    for (size_t i = Nr - 1; i > 1; i -= 2) {
      ...
      state =
          vaesimcq_u8(vaesdq_u8(state, vaesimcq_u8(neon_round_keys[i - 1])));
    }
